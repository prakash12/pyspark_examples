...
spark.sql("SELECT * FROM {df} WHERE {df[age]} > :num1 AND {df[age]} <= :num2 AND {df[address]} LIKE :lk", {"num1" : 20, "num2" : 45, 'lk' : "%ore%"}, df=mydf).show()
#Get all employees age between 20 and 45 and address not contains ore
spark.sql("SELECT * FROM {df} WHERE {df[age]} > :num1 AND {df[age]} <= :num2 AND {df[address]} NOT LIKE :lk", {"num1" : 20, "num2" : 45, 'lk' : "%ore%"}, df=mydf).show()
...
spark.sql("SELECT department, SUM(salary) AS total_salary,MAX(salary) AS max_salary,MIN(salary) AS min_salary,MEAN(salary) AS mean_salary FROM {df} GROUP BY {df[department]}", df=mydf).show()
